...
public class PhnumAuthServiceTwilio implements PhnumAuthService {
    ...
    public SendCodeResDto sendCode(final String phoneNumber) {
        ...
        if(Objects.isNull(messageSid)){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "인증번호 발송 실패");
        }
    ...
    private boolean checkAuthCode(String phNumber, Integer code){
        if(Objects.isNull(phNumber) || Objects.isNull(code)){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "요청 값이 Null입니다.");
        }
        PhnumAuthCode persist = repo.findById(phNumber)
                                .orElseThrow(()->
                                    new ResponseStatusException(HttpStatus.BAD_REQUEST, "휴대폰 인증 요청 기록이 없습니다."));
        ...
        if(!isValidTime){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "인증 시간을 초과했습니다.");
        }
        if(!persist.getAuthCode().equals(code)){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "인증코드가 틀렸습니다");
        }
        return true;
